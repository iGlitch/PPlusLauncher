...
xd3_getblk (xd3_stream *stream, xoff_t blkno)
{
  ...
  if (source->curblk == NULL || blkno != source->curblkno)
      ...
      if (stream->getblk == NULL)
	{
      ...
      if (ret != 0)
	{
		  IF_DEBUG1(DP(RINT "[getblk] app error blkno %"XDELTAQ"u: %s\n",
  ...
  if (blkno >= source->frontier_blkno)
      ...
      if (source->onblk == source->blksize)
	{
	  ...
	  IF_DEBUG2(DP(RINT "[getblk] full source blkno %"XDELTAQ"u: "
		  "source length unknown %"XDELTAQ"u\n",
			blkno,
  ...
  IF_DEBUG2(DP(RINT "[getblk] read source block %"XDELTAQ"u onblk %u blksize %u\n",
  ...
  if (ret == 0)
    {
	  ...
	  IF_DEBUG2(DP(RINT "[set source] size known %"XDELTAQ"u\n",
		    source_size));
...
xd3_iopt_finish_encoding (xd3_stream *stream, xd3_rinst *inst)
{
  ...
  switch (inst->type)
	...
	IF_DEBUG2 ({
	  static int cnt;
	  DP(RINT "[iopt copy:%d] pos %"XDELTAQ"u-%"XDELTAQ"u addr %"XDELTAQ"u-%"XDELTAQ"u size %u\n",
		   cnt++,
	...
	IF_DEBUG2 ({
	  static int cnt;
	  DP(RINT "[iopt run:%d] pos %"XDELTAQ"u size %u\n", cnt++, stream->total_in + inst->pos, inst->size);
	});
	break;
      }
    case XD3_ADD:
	...
	IF_DEBUG2 ({
	  static int cnt;
	  DP(RINT "[iopt add:%d] pos %"XDELTAQ"u size %u\n", cnt++, stream->total_in + inst->pos, inst->size);
	});
...
xd3_encode_input (xd3_stream *stream)
{
  ...
  switch (stream->enc_state)
	  ...
	  (stream->avail_in < stream->winsize && ! (stream->flags & XD3_FLUSH)))
	{
	  ...
	  IF_DEBUG2(DP(RINT "[WINSTART:%"XDELTAQ"u] input bytes %u offset %"XDELTAQ"u\n",
      ...
      if ((stream->enc_current = stream->enc_current->next_page) != NULL)
	{
	  ...
	  IF_DEBUG2(DP(RINT "[WINFINISH:%"XDELTAQ"u] in=%"XDELTAQ"u\n",
...
xd3_source_match_setup (xd3_stream *stream, xoff_t srcpos)
{
  ...
  IF_DEBUG2(DP(RINT "[match_setup] frontier_pos %"XDELTAQ"u, srcpos %"XDELTAQ"u, "
	  "src->max_winsize %"XDELTAQ"u\n",
  ...
  if (srcpos < frontier_pos &&
    ...
    IF_DEBUG1(DP(RINT "[match_setup] rejected due to src->max_winsize "
		"distance eof=%"XDELTAQ"u srcpos=%"XDELTAQ"u maxsz=%"XDELTAQ"u\n",
		 xd3_source_eof (stream->src),
  ...
  if (stream->srcwin_decided == 0)
      ...
      if (stream->src->eof_known)
	{
      ...
      IF_DEBUG2(DP(RINT
		  "[match_setup] srcpos %"XDELTAQ"u (tgtpos %"XDELTAQ"u) "
...
xd3_source_extend_match (xd3_stream *stream)
{
  ...
  IF_DEBUG2(DP(RINT "[extend match] srcpos %"XDELTAQ"u\n",
	       stream->match_srcpos));
  ...
  if (stream->match_state == MATCH_BACKWARD)
      ...
      while (stream->match_back < stream->match_maxback)
	{
	  ...
	  IF_DEBUG2(DP(RINT "[maxback] maxback %u trysrc %"XDELTAQ"u/%u tgt %u tryrem %u\n",
  ...
  if (stream->match_fwd < stream->min_match)
      ...
      if (match_end > stream->maxsrcaddr)
	{
      ...
      IF_DEBUG2 ({
	static int x = 0;
	DP(RINT "[source match:%d] <inp %"XDELTAQ"u %"XDELTAQ"u>  <src %"XDELTAQ"u %"XDELTAQ"u> (%s) [ %u bytes ]\n",
	   x++,
...
xd3_srcwin_move_point (xd3_stream *stream, usize_t *next_move_point)
{
  ...
  while (stream->srcwin_cksum_pos < logical_input_cksum_pos &&
      ...
      if ((ret = xd3_getblk (stream, blkno)))
	{
	  ...
	  IF_DEBUG1 (DP(RINT
		  "[srcwin_move_point] async getblk return for %"XDELTAQ"u\n",
			blkno));
	  return ret;
	}

      IF_DEBUG1 (DP(RINT
		  "[srcwin_move_point] T=%"XDELTAQ"u{%"XDELTAQ"u} S=%"XDELTAQ"u EOF=%"XDELTAQ"u %s\n",
		    stream->total_in + stream->input_position,
  ...
  IF_DEBUG1 (DP(RINT
	  "[srcwin_move_point] exited loop T=%"XDELTAQ"u{%"XDELTAQ"u} "
	  "S=%"XDELTAQ"u EOF=%"XDELTAQ"u %s\n",
		stream->total_in + stream->input_position,
		logical_input_cksum_pos,
		stream->srcwin_cksum_pos,
		xd3_source_eof (stream->src),
