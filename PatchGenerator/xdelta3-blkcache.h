  ...
  if (option_verbose)
    {
      ...
      if (sfile->size_known)
	{
		  short_sprintf(srcszbuf, "source size %s [%"XDELTAQ"u]",
...
main_read_seek_source (xd3_stream *stream,
  ...
  if (sfile->seek_failed || ret != 0)
	  ...
	  if (!option_quiet)
	    {
	      XPR(NT "source can't seek backwards; requested block offset "
			  "%"XDELTAQ"u source position is %"XDELTAQ"u\n",
      ...
      if (!sfile->seek_failed && option_verbose)
	{
	  ...
	  if (option_verbose > 1)
	    {
		  XPR(NT "seek error at offset %"XDELTAQ"u: %s\n",
	...
	{
		  XPR(NT "non-seekable source skipping %"XDELTAQ"u bytes @ %"XDELTAQ"u\n",
	      pos - sfile->source_position,
      ...
      while (sfile->source_position < pos)
	{
	  ...
	  IF_DEBUG1(DP(RINT "[getblk] skip blkno %"XDELTAQ"u size %u\n",
		  ...
		  xoff_t      blkno)
{
  ...
  if (option_verbose > 3)
      ...
      if (blru->blkno != (xoff_t)-1)
	{
	  if (blru->blkno != blkno)
	    {
		  XPR(NT "source block %"XDELTAQ"u read %zu ejects %"XDELTAQ"u (lru_hits=%u, "
		  "lru_misses=%u, lru_filled=%u)\n",
		  blkno, nread, blru->blkno, lru_hits, lru_misses, lru_filled);
	    }
	  else
	    {
		  XPR(NT "source block %"XDELTAQ"u read %zu (lru_hits=%u, "
		  "lru_misses=%u, lru_filled=%u)\n",
	...
	{
		  XPR(NT "source block %"XDELTAQ"u read %zu (lru_hits=%u, lru_misses=%u, "
	      "lru_filled=%u)\n", blkno, nread, 
  ...
  IF_DEBUG1(DP(RINT "[main_getblk] blkno %"XDELTAQ"u onblk %zu pos %"XDELTAQ"u "
	  "srcpos %"XDELTAQ"u\n",
