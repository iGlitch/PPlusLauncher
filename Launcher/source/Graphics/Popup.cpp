...
void Popup::draw()
{
	...
	for (int i = 0; i < m_iSelectionCount; i++)
	{
		...
		{
			Menu_DrawRectangle(xPos + ((f32)i * width / (f32)m_iSelectionCount), yPos + (height * m_fTextAreaRatio), width / (f32)m_iSelectionCount, height - (height * m_fTextAreaRatio), (GXColor){ 163, 255, 224, u8(255 * animationRatio) }, true);
