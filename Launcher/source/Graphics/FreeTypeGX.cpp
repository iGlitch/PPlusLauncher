...
void setLoadFlags(FT_Int32 value)
{
	load_flags = value;
}

FT_Int32 getLoadFlags()
{
	return load_flags;
}

void setRenderMode(FT_Render_Mode value)
{
	render_mode = value;
}

FT_Render_Mode getRenderMode()
{
	return render_mode;
}
FreeTypeGX *fontSystem[MAX_FONT_SIZE + 1];

void InitFreeType(const uint8_t* fontBuffer, FT_Long bufferSize)
{
	FT_Init_FreeType(&ftLibrary);
	FT_New_Memory_Face(ftLibrary, (const FT_Byte *)fontBuffer, bufferSize, 0, &ftFace);
	ftSlot = ftFace->glyph;
...
ftgxCharData *FreeTypeGX::cacheGlyphData(wchar_t charCode)
{
	...
	if (!FT_Load_Glyph(ftFace, gIndex, load_flags))
	{
		FT_Render_Glyph(ftFace->glyph, render_mode);
		if (ftSlot->format == FT_GLYPH_FORMAT_BITMAP)
		{
			...
			ftgxCharData data;
			data.renderOffsetX 		= ftSlot->bitmap_left;
			data. glyphAdvanceX 	= ftSlot->advance.x >> 6;
			data. glyphIndex 		= gIndex;
			data. textureWidth 		= textureWidth;
			data. textureHeight 	= textureHeight;
			data.renderOffsetY 		= ftSlot->bitmap_top;
			data.renderOffsetMax 	= ftSlot->bitmap_top;
			data.renderOffsetMin 	= glyphBitmap->rows - ftSlot->bitmap_top;
			data.glyphDataTexture 	= NULL;

			this->fontData[charCode] = data;
...
uint16_t FreeTypeGX::drawText(int16_t x, int16_t y, wchar_t *text, GXColor color, uint16_t textStyle)
{
	...
	while (text[i])
	{
		...
		if (glyphData != NULL)
		{
			...
			//drawTexturedBox(&glyphTexture, x_pos + glyphData->renderOffsetX + x_offset, y - glyphData->renderOffsetY + y_offset, glyphData->textureWidth, glyphData->textureHeight, 255, 255, 255, 255);
...
{
	drawTexturedBox(texObj, screenX, screenY, texWidth, texHeight, color.r, color.g, color.b, color.a);
...
void FreeTypeGX::copyFeatureToFramebuffer(f32 featureWidth, f32 featureHeight, int16_t screenX, int16_t screenY, GXColor color)
{
	...
	drawBox(screenX, screenY, featureWidth, featureHeight, color.r, color.g, color.b, color.a);
