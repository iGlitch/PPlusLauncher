...
typedef struct {
	const u8 * streamfile; /* file used by this channel */
...
static inline int32_t read_32bitBE(off_t offset, const u8 * s)
{
	uint8_t buf[4];

	memcpy(buf, s + offset, 4);

	return get_32bitBE(buf);
}

static inline int8_t read_8bit(off_t offset, const u8* s)
{
	uint8_t buf[1];

	memcpy(buf, s + offset, 1);

	return buf[0];
}

static inline int16_t read_16bitBE(off_t offset, const u8*s)
{
	uint8_t buf[2];

	memcpy(buf, s + offset, 2);
...
protected:
	...
	s32 trackLength; //!< Length of sound data
